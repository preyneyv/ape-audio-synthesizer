...
#include <utils/q1x15.h>
#include <utils/q1x31.h>

#include "synth.h"

audio_synth_t g_synth;
...
void audio_synth_fill_buffer(audio_synth_t *synth, int32_t *buffer,
                             uint32_t buffer_size)
{
    // lock the config
    ...
    for (uint32_t i = 0; i < buffer_size; i++)
    {
        ...
        buffer[i] = (int32_t)((int16_t)sample);
    }
